...
from multi_modal_heart.common.scheduler import CosineWarmupScheduler
...
class LitAutoEncoder(pl.LightningModule):
    def __init__(self, network,task_name,input_key_name="input_seq", target_key_name="cleaned_seq", future_key_name="next_seq", grad_clip=False, warmup=50,
        max_iters=2000,batch_size=128, args=None,**kwargs):
        super().__init__()
    ...
    def get_text_embedding(self, text):
        assert self.text_tokenizer is not None, "text tokenizer is not defined"
        assert self.text_model is not None, "text model is not defined"
        inputs = self.text_tokenizer(text, max_length = 30, padding = 'max_length', return_tensors="pt", truncation = True)
        
        hidden_state = output[0][0].detach()
        embedding = torch.mean(hidden_state,keepdim=True,dim=1)
        p
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.AdamW(filter(lambda p: p.requires_grad, self.parameters()), lr=1e-3, betas=(0.9, 0.95))
...
class LitClassifier(pl.LightningModule):
    def __init__(self,encoder,input_dim,num_classes=5,learning_rate=1e-3, encoder_lr=1e-3,freeze_encoder=False, gradual_unfreeze=True,
                 task_name = "ECG_Classifier", max_iters =20000,output_dir="./",**kwargs):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        self.lr_scheduler = CosineWarmupScheduler(
            optimizer, warmup=50, max_iters=self.max_iters
            )
        ## one cylce lr
...
def cli_main():
    ...
    if ae_type =="dae":
        ...
        latent_code_dim = 512
    elif ae_type=="ECG_attention_512_raw_no_attention_pool_abl_no_lead_attention":
        ecg_net = ECGAttentionAE(num_leads=n_leads, time_steps=max_seq_len, z_dims=512, linear_out=64, downsample_factor=5, base_feature_dim=4,if_VAE=False,use_attention_pool=False, no_lead_attention=True)
        latent_code_dim = 512
    elif ae_type=="ECG_attention_512_raw_no_attention_pool_abl_no_time_attention":
        ecg_net = ECGAttentionAE(num_leads=n_leads, time_steps=max_seq_len, z_dims=512, linear_out=64, downsample_factor=5, base_feature_dim=4,if_VAE=False,use_attention_pool=False, no_time_attention=True)
        latent_code_dim = 512
    ...
    ## save result to csv
    # result.to_csv(os.path.join(tb_logger.log_dir,"result.csv"))
    ...
    if args.finetune:
        ...
        classifier = LitClassifier(encoder,input_dim = latent_code_dim, num_classes=5,learning_rate=1e-3,encoder_lr=args.encoder_lr,
                                   freeze_encoder=freeze_encoder, task_name = finetune_task_name,
                                   max_iters=finetune_max_epochs * len(train_loader),output_dir=output_dir,gradual_unfreeze=gradual_unfreeze)
