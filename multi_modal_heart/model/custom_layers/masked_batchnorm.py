## implement the masked batch normalization
...
def masked_batch_norm(input: Tensor, mask: Tensor, weight: Optional[Tensor], bias: Optional[Tensor],
    ...
    if training:
        ...
        ## only update the running mean and variance where mask is not zero
        print(f'mean shape, {mean.shape}')
        mean [mask.sum(_dims)==0] = running_mean[mask.sum(_dims)==0]
        var [mask.sum(_dims)==0] = running_var[mask.sum(_dims)==0]
