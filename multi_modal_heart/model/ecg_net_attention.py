...
# time_encoder = PositionalEncoding(d_model = signal_length,dropout=0.5)
# class STAttentionModel(nn.Module):
#     def __init__(self,n_feature=256, num_leads = 12, signal_length=1024 ,out_feature = 64,use_attention_mask=False, use_attention_pool=True, no_time_attention=False,no_lead_attention=False,apply_subsequent_attention=False, apply_lead_mask=False):
#         '''
...
#             self.time_encoder = PositionalEncoding(d_model =self.n_feature,dropout=0.2)
#         else: 
#             self.time_mha=None
#             self.time_encoder = None
#         self.norm1 = nn.LayerNorm(self.n_feature)
...
#             self.lead_encoder= None
#         self.norm2 = nn.LayerNorm(lead_feature_embed)

#         ## define the final attention for feature aggregation and pooling
#         self.use_attention_pool = use_attention_pool
#         if use_attention_pool:
#             self.attention_pool = AttentionPool1d(spacial_dim=signal_length, embed_dim=n_feature*num_leads, num_heads=8, output_dim=out_feature, dropout_rate=0.2)
#         else:
...
#                 ## pre-ln
#                 lead_input_w_time_encodin_normed =self.norm1(lead_input_w_time_encoding)
#                 lead_out, lead_attention_mask = self.time_mha[i](lead_input_w_time_encodin_normed,
#                                                                 lead_input_w_time_encodin_normed,
#                                                                 lead_input_w_time_encodin_normed,
#                                                                 attn_mask=self.attn_mask)
#                 lead_out =lead_out+lead_input_w_time_encoding
#                 lead_time_output.append(lead_out)
#                 lead_attention_masks.append(lead_attention_mask)
...
#             # lead_input_w_lead_encoding =self.lead_dropout(lead_input+self.lead_embedding)
#             lead_input_w_lead_encoding = self.lead_encoder(lead_input)
#             lead_input_w_lead_encoding_normed = self.norm2(lead_input_w_lead_encoding)
...
#             else:
#                 lead_attn_mask = None
#             cross_lead,lead_attn_output_weights = self.lead_mha(lead_input_w_lead_encoding_normed,lead_input_w_lead_encoding_normed,lead_input_w_lead_encoding_normed,attn_mask=lead_attn_mask)
#             ## residual connection
...
#         else:
#             cross_lead_time_flattenn  =self.attention_pool(cross_lead_sum)

#         self.lead_attention = lead_attn_output_weights
#         self.time_attention = lead_attention_masks ## temporal attention masks for each lead,  (signal_length, batch_size, n_features)
